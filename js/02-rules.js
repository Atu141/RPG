...
(function(){
  ...
  window.ConditionUI={renderMaster:renderMasterPanel,renderPlayer:function(){if(!selectedPlayer||!$('#sheetContent'))return;let host=$('#playerConditionsV059');if(!host){host=document.createElement('div');host.id='playerConditionsV059';host.className='panel player-conditions-v059';$('#sheetContent')?.prepend(host);}host.innerHTML=`<div class="panel-title"><div><span class="icon">◉</span><div><h2>Condições</h2><p>Estados atuais do personagem.</p></div></div></div><div class="condition-chips">${ConditionEngine.renderPlayer(selectedPlayer)}</div>`;}};
})();
